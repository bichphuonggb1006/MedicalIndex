...
class ScheduleCtrl extends Controller
{
    ...
    public function __construct(MvcContext $context)
    {
    ...
    function getSchedule($siteFK, $id)
    {
        $siteFK= 'master';
        Auth::getInstance()->requireLogin();

        $row = ScheduleMapper::makeInstance()
            ->filterSite($siteFK)
            ->autoloadFile()->filterID($id)
            ->getEntity();

        $this->outputJSON($row);
    }

    function getFile($siteID, $id)
    {
        $siteID= 'master';
        Auth::getInstance()->requireLogin();

        $file = FileMapper::makeInstance()
            ->filterSiteID($siteID)
            ->filterID($id)->getEntityOrFail();
        $file->outputFile();
    }
    ...
    function newRequest($siteFK)
    {
        $siteFK= 'master';
        $patientData = $this->input('patient');
        $patientEntity = \Teleclinic\Api\Auth\Auth::makeInstance()->requireLogin()->user();
        //nếu bệnh nhân chưa cập nhật thông tin cá nhận thì tự động lưu thông tin cá nhân
        if (empty($patientEntity->fullName))
        {
            $patientData['siteID'] = $siteFK;
            $rs = PatientMapper::makeInstance()->storeOrUpdate($patientData, $patientEntity->id);
            if($rs['status'] ==false)
            {
                return  $this->outputJSON($rs);
            }
            $patientEntity = $rs['data'];
        }

        $patientEntity->age = arrData($this->input('patient'), 'age');
        $reqServiceIDs = arrData($this->input(), 'reqServiceIDs', []);
        //TH cũ, ko phải TH nhiều DV
        if (!count($reqServiceIDs)) {
            $inputs = $this->input();
            $inputs['patientID'] = $patientEntity->id;
            $inputs['patientCode'] = $patientEntity->code;
            $inputs['patient'] = array_merge((array)$patientEntity ,$patientData);
            $inputs['reqType'] = arrData($inputs, 'reqType', ScheduleMapper::REQUEST_TYPE_KHACH_LE);
            $inputs['siteID'] = $siteFK;
        ...
        foreach ($requestTimes as $srvID => $req) {
            $inputs = [
                ...
                'patient' => array_merge((array)$patientEntity ,$patientData),
                'reqDate' => arrData($req, 'reqDate'),
                'reqType' => arrData($req, 'reqType', ScheduleMapper::REQUEST_TYPE_KHACH_LE),
    ...
    function confirmSchedule($siteID, $id)
    {
        $siteID= 'master';
        Auth::getInstance()->requireLogin();
        ...
        ScheduleMapper::makeInstance()
            ->filterSite($siteID)
            ->confirmSchedule($id, $this->input('scheduledDate'), $this->input('vclinicID'));
        $this->resp->setBody(result());
    }

    function diagnosis($siteID, $id)
    {
        $siteID= 'master';
        Auth::getInstance()->requireLogin();
        ...
        ScheduleMapper::makeInstance()->diagnosis($siteID, $id, $input);
        $this->resp->setBody(result());
    }

    function cancel($siteID, $id)
    {
        $siteID= 'master';
        Auth::getInstance()->requireLogin();

        ScheduleMapper::makeInstance()->cancel($siteID, $id, $this->input('comment'));
        $this->resp->setBody(result());
    }
...
//            throw new \Exception("Chưa thanh toán");
    ...
    function getTreatmentProcess()
    {
        ...
        $patient = PatientMapper::makeInstance()->filterID($patientID)->getEntityOrFail();

        $schedules = ScheduleMapper::makeInstance()->filterPatientID($patient->id)
            ->filterTreatmentType('covid')
            ->filterStatus(['unscheduled'])
            ->orderBy("id desc")
            ->getAll()
            ->toArray();
